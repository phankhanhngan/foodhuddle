...
export const getGroupFoodOrderQuery = (
  ...
  const calculateOrderTotalQuery = `SUM(food_order_total) + SUM(option_total)`;
  switch (groupedBy) {
    case GroupedBy.food:
      return `
        SELECT 
            ...
            ,${calculateOrderTotalQuery} AS total
            FROM 
            ${joinedTableQuery}
    ...
    case GroupedBy.user:
      return `
        SELECT 
            ...
            ,${calculateOrderTotalQuery} AS totalPayment 
            ,CAST(
                (${calculateOrderTotalQuery}) + (MAX(fee) / (COUNT(user_id) OVER ())) AS DECIMAL(64,2)
            ) AS finalPayment
            FROM 
            ${joinedTableQuery}
            GROUP BY user_id
